...
- Catalog bumps land in the root `workspaces.catalog` block and `bun.lock` together; a Renovate PR touching only the catalog still exercises every consumer because `verify` builds all workspaces.
