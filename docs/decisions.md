...
- **Routing.** A pathless `_shell` layout route; routes carry the change slug:
  `/changes/$changeId[/documents|/design-docs[/$docId]]`, plus
  `/system-model`. `/` redirects to the last-opened change
...
- **The frontend is partitioned by domain, not by layer**, and Oxlint's
  `boundaries` enforces it the way it enforces the backend's layers (D3).
  `src/features/<domain>/` owns one domain, named after the backend feature it
  faces (`changes`, `design-docs`, `documents`, `system-model`): its
  queries in `<domain>.api.ts`, its vocabulary in `<domain>.model.ts`, its
  components in `ui/`, one per file, the screen a route renders named
  `<domain>-view.tsx`. `src/shell/` is the chrome — layout, header, sidebar,
  view header and the navigation items — and the frontend's counterpart to the
  backend's `ui/`. `src/shared/` is the kernel: the typed client, the design
  system, the query wiring, the route ids. `src/routes/` is wiring only.
- **The direction of dependency:** the shell composes features and no feature
  imports the shell; a feature reaches another feature through its `*.api.ts`
  or `*.model.ts` and never through its `ui/`, which is why a feature's own
  files are imported relatively and the `#/` alias means crossing a module;
  `shared/` knows no feature; `routes/` may reach anything and nothing but the
  generated tree imports it.
- **Route files export `Route` and nothing else** (Oxlint's
  `react/only-export-components`), with a literal route id: the router
  generator accepts no imported constant there. `shared/routing/route-ids.ts`
  holds the same ids `satisfies AppRouteIds`, so what the shell matches on is
  checked against the generated tree.
- **The shell is change-scoped:** a picker at the top of a 280 px sidebar names
  the current change; under it the change views (Overview, Documents, Design
  docs, the last listing the change's documents beneath it); a pinned
  "Documentation" zone holds the System model. The shell renders the view
  header once, above the outlet. Below `md` the sidebar is Mantine's
  `AppShell` drawer.
