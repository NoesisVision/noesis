...
Services own use-case orchestration; both entry points (MCP tools in `adapters/mcp`, HTTP routes in `ui/`) call them and nothing bypasses them. Design documents are authored through the MCP tools alone — the `ui/design-docs` surface reads and deletes, it has no create or update route (decision D3). MCP tools are thin: validate input, call one service method. Large payloads never travel inline — the agent writes a file under `.noesis/tmp/<session>/` and passes the path. A tool that reads such a file checks it against its contract in `app/validation/contracts/` before the service sees it; there is no separate validating tool. Errors are returned in-band (`isError`), never as protocol errors, and carry the actionable issue list (path, expected versus found, one-line fix).
