...
[`design-doc.md`](design-doc.md); decisions 19 and 20 in
[`docs/decisions.md`](../../docs/decisions.md) record the engine and the
graph vocabulary.
...
| `core/`         | `noesis-scanner-core`         | The engine: ArchUnit's `ClassFileImporter` over a classes directory, stereotype detection through a configurable annotation mapping (decision 20), derivers for modules, behaviours, invocations, message edges and port bindings, and a Jackson JSON writer                |
...
Decision 20's typed vocabulary (design-doc §9.4), as the `NodeType` and
...
Not implemented yet from decision 19: the Spoon source-fidelity pass
...
Not a bun workspace member; Maven owns this directory (decision 21).
