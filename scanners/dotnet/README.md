...
writes the same typed building-block graph, decision D9's node and edge
...
- **Layout**: a self-contained subtree with its own toolchain, outside the
  bun workspace, like `scanners/java` with its `pom.xml` (decision D7).
...
- **Integration**: not with this migration. The service runs only the
  ...
  settles (decision D9). Until then a .NET scanner would
