# frontend

The Noesis web UI: a client-only React SPA on TanStack Router with Mantine
(decisions 63 and 67). There is no SSR and no server-side execution of any
kind — anything that needs a server is a Hono route on the backend's `/ui`
surface.

This package has no build of its own. The backend imports `index.html`, and
bun bundles what the page references: on the fly with hot reload when the
service runs from source, ahead of time into the service's `dist/` when it is
built. Run it from the repo root with `bun run dev` (the service on `:3000`).

## Scripts

| Script                    | What it does                      |
| ------------------------- | --------------------------------- |
| `bun run check-types`     | `tsc --noEmit`                    |
| `bun run generate-routes` | Regenerate `src/routeTree.gen.ts` |

## Layout

```
index.html                  # the page the backend imports; mounts #app
src/
  main.tsx                  # providers (Mantine, Query), router, mount
  router.tsx                # createRouter over the generated route tree
  routes/                   # file-based routes; each file exports only `Route`
  components/               # route components and shared UI
  integrations/tanstack-query/
  styles.css                # the few global rules Mantine does not set
  env.d.ts                  # module declarations for side-effect CSS imports
```

Route files export `Route` and nothing else — biome's
`useComponentExportOnlyModules` rejects both extra exports and unexported
components there. View components therefore live in `src/components/`.

`src/routeTree.gen.ts` is generated by `tsr generate`; it is committed, so
run `bun run generate-routes` after adding or renaming a route.
