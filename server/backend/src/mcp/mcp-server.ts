...
export function createMcpServer(deps: McpDeps): Server {
  ...
  const tools = {
    ...
    'scan-system-model': define({
      description:
        'Scans the repository source code and writes the implemented model to .noesis/system-model/, one file per unit: bounded contexts, modules, building blocks (typed by their DDD stereotype annotations where the code has them) and their behaviours, each with its source location. Every supported language is scanned; a polyglot repository needs no configuration. Run it before designing against existing code, or when the system model is missing or stale.',
      args: z.object({}),
      handler: async () => {
        ...
        return text(
          lines.length === 0
            ? 'No units found under the repository root (no package or module of a supported language with source files).'
