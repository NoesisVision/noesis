// Regenerates each plugin's skills/*/references/*.json from src/mcp/contracts.
// Run via `bun run generate`. Output is committed so plugins stay
// self-contained when installed outside this monorepo; the service owns the
// contracts, so it also owns emitting the model-facing reference JSONs.
import { mkdir, writeFile } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import { contracts, toJsonSchema } from '../src/mcp/contracts/index.js';
